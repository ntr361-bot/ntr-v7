...
namespace 六合分析软件
{
    ...
    public static class OpenAIService
    {
        ...
        private const int DefaultDailyAnalysisLimit = 4;
        ...
        public static async Task<GptAnalysisResult> AnalyzeAsync(string prompt,
            ZodiacPredictEngine.PredictResult? localResult, string localAnalysis = "")
        {
            ...
            if (string.IsNullOrEmpty(ApiKey))
            {
                ...
                result.AnalysisText = BuildFallbackAnalysis(localAnalysis, localResult, "未配置GPT API Key");
                result.Recommended6 = localResult?.Top6 ?? new List<string>();
                result.Focus3 = localResult?.Top3 ?? new List<string>();
                return result;
            }

            if (!TryReserveDailyAttempt(out int dailyLimit))
            {
                result.Success = true;
                result.UsedFallback = true;
                result.AnalysisText = BuildFallbackAnalysis(localAnalysis, localResult,
                    $"已达到每日GPT润色上限{dailyLimit}次");
                result.Recommended6 = localResult?.Top6 ?? new List<string>();
                result.Focus3 = localResult?.Top3 ?? new List<string>();
                return result;
            }
        ...
        private static bool TryReserveDailyAttempt(out int limit)
        {
            limit = DefaultDailyAnalysisLimit;
            string? configured = Environment.GetEnvironmentVariable("OPENAI_DAILY_ANALYSIS_LIMIT");
            if (int.TryParse(configured, out int parsed))
                limit = Math.Clamp(parsed, 0, 100);
            if (limit == 0) return false;

            lock (UsageLock)
            {
                string today = DateTime.Today.ToString("yyyy-MM-dd");
                JsonFileCache.TryLoad<DailyUsageState>("openai-daily-usage", today, out var state);
                state ??= new DailyUsageState();
                if (state.Attempts >= limit) return false;
                state.Attempts++;
                JsonFileCache.Save("openai-daily-usage", today, state);
                return true;
            }
        }

        private static string BuildFallbackAnalysis(string localAnalysis,
            ZodiacPredictEngine.PredictResult? localResult, string reason)
        {
            if (!string.IsNullOrWhiteSpace(localAnalysis))
                return $"【分析来源】本地智能统计（{reason}，预测排序不受影响）{Environment.NewLine}{Environment.NewLine}{localAnalysis}";
            return $"【分析来源】本地统计（{reason}，预测排序不受影响）{Environment.NewLine}" +
        ...
        {
            return AnalyzeAsync(prompt, localResult, localAnalysis).GetAwaiter().GetResult();
        }
    }
}
