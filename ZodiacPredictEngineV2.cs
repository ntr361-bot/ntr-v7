...
namespace 六合分析软件
{
    ...
    public class ZodiacPredictEngineV2
    {
        ...
        {
            // 主预测数据是新到旧，回测必须先转为旧到新再切分训练集和测试集。
            var chronological = zodiacData.AsEnumerable().Reverse().ToList();
            int trainPeriods = Math.Min(100, chronological.Count / 2);
            int testPeriods = Math.Min(30, chronological.Count - trainPeriods);
            if (testPeriods < 5) return 0;

            int firstWindowStart = Math.Max(0, chronological.Count - trainPeriods - testPeriods * 3);
            var windowScores = new List<double>();
            for (int windowStart = firstWindowStart;
                 windowStart + trainPeriods < chronological.Count && windowScores.Count < 3;
                 windowStart += testPeriods)
            {
                int windowTestCount = Math.Min(testPeriods, chronological.Count - (windowStart + trainPeriods));
                if (windowTestCount <= 0) continue;

                int top3Hits = 0;
                int top6Hits = 0;
                for (int i = 0; i < windowTestCount; i++)
                {
                    int testPos = windowStart + trainPeriods + i;
                    string actual = chronological[testPos];
                    var trainData = chronological
                        .Take(testPos)
                        .Reverse()
                        .ToList();
                    ...
                    if (scores.Take(3).Any(s => s.zodiac == actual)) top3Hits++;
                    if (scores.Take(6).Any(s => s.zodiac == actual)) top6Hits++;
                }

                double top3Rate = (double)top3Hits / windowTestCount;
                double top6Rate = (double)top6Hits / windowTestCount;
                windowScores.Add(top3Rate * 0.6 + top6Rate * 0.4);
            }

            return windowScores.Count > 0 ? windowScores.Average() : 0;
        }
